...
class AdvancedAnalysisService {
    ...
    async getAnalytics(doctorId, dateRange = "30d", filter) {
        ...
        const rangeStart = dateRange === "7d" ? new Date(now.getTime() - 7 * 24 * 60 * 60 * 1000) :
        ...
        const investigationCompletionRate = totalOrdered > 0 ? Math.round((totalDoneWithReport / totalOrdered) * 100) : 0;
        const patientsNeedingAction = rows.filter((r) => r.issueCount > 0).length;
        const criticalSafetyFlags = rows.filter((r) => r.issues.includes("CLIPS_MISSING") || r.issues.includes("CHEMO_ISSUE")).length;
        // ── Stage Distribution ───────────────────────────────────────────────
        const stageCountsMap = {
            REGISTERED: 0, INVESTIGATIONS: 0, TREATMENT_PLANNED: 0, NACT: 0, SURGERY: 0,
        };
        for (const row of rows)
            stageCountsMap[row.currentStage]++;
        const stageDistribution = Object.entries(stageCountsMap).map(([stage, count]) => ({
            stage,
            count,
        ...
        for (const row of rows) {
            for (const inv of Object.values(row.investigations)) {
                ...
                invTotal++;
                if (inv.testDone && inv.reportCollected)
                    invStatusMap.DONE_REPORT++;
                else if (inv.testDone && !inv.reportCollected)
                    invStatusMap.DONE_NO_REPORT++;
                else if (inv.appointmentStatus === "MISSED")
                    invStatusMap.MISSED++;
                else if (inv.appointmentStatus === "SCHEDULED")
                    invStatusMap.SCHEDULED++;
                else
                    invStatusMap.PENDING++;
            }
        }
        const investigationCompletion = Object.entries(invStatusMap).map(([status, count]) => ({
            status,
            count,
        ...
        for (const row of rows) {
            ...
            if (radiotherapy)
                treatMap.RT_ADDED++;
            else if (nact && surgeryType === "BCS")
                treatMap.NACT_BCS++;
            else if (nact && surgeryType === "MASTECTOMY")
                treatMap.NACT_MASTECTOMY++;
            else if (!nact && surgeryType === "BCS")
                treatMap.BCS_ONLY++;
            else if (!nact && surgeryType === "MASTECTOMY")
                treatMap.MASTECTOMY_ONLY++;
            else
                treatMap.NOT_PLANNED++;
        }
        const treatmentBreakdown = Object.entries(treatMap).map(([plan, count]) => ({
            plan,
            count,
            percentage: total > 0 ? Math.round((count / total) * 100) : 0,
        }));
        ...
        for (const row of rows) {
            for (const issue of row.issues) {
                if (issue === "MISSED_APPT")
                    issueMap.MISSED_APPOINTMENT++;
                else if (issue === "NO_REPORT")
                    issueMap.NO_REPORT++;
                else if (issue === "CHEMO_ISSUE")
                    issueMap.CHEMO_ISSUE++;
                else if (issue === "CLIPS_MISSING")
                    issueMap.CLIPS_MISSING++;
                else if (issue === "NO_PLAN")
                    issueMap.NO_TREATMENT_PLAN++;
            }
        }
        ...
        // ── Gender Distribution ──────────────────────────────────────────────
        const genderMap = { FEMALE: 0, MALE: 0, OTHER: 0 };
        for (const row of rows) {
        ...
        const investigationHeatmap = advancedAnalysisTypes_1.INVESTIGATIONS.map(({ key, label }) => {
            ...
            for (const row of rows) {
                ...
                if (inv.testDone && inv.reportCollected)
                    ...
                    missed++;
                else if (inv.appointmentStatus === "CANCELLED")
                    cancelled++;
                else if (inv.appointmentStatus === "SCHEDULED")
                    scheduled++;
                else
        ...
        function getActivityType(pageNumber) {
            if (pageNumber === 29)
                return "UPDATE";
            if (pageNumber >= 30 && pageNumber <= 36)
                return "UPDATE";
            if (pageNumber === 37 || pageNumber === 38)
        ...
        function getPageLabel(pageNumber) {
            ...
            if (pageNumber >= 7 && pageNumber <= 28)
                return `Investigation page ${pageNumber}`;
            if (pageNumber === 29)
                return "Treatment Plan";
            if (pageNumber >= 30 && pageNumber <= 36)
                return "NACT Tracking";
            if (pageNumber === 37)
                return "Radiation Booking";
            if (pageNumber === 38)
                return "Surgery Admission";
        ...
        for (const inv of advancedAnalysisTypes_1.INVESTIGATIONS)
            INV_LABEL_MAP[inv.key] = inv.label;
        ...
        for (const row of rows) {
            for (const [invKey, inv] of Object.entries(row.investigations)) {
                ...
                const diffDays = Math.floor((apptDay.getTime() - todayStart.getTime()) / 86400000);
                // Assign dateGroup:
                //   overdue (past) → TODAY (needs urgent follow-up)
                //   today          → TODAY
                //   tomorrow       → TOMORROW
                //   future         → THIS_WEEK
                const dateGroup = diffDays <= 0 ? "TODAY" :
                    diffDays === 1 ? "TOMORROW" :
                        "THIS_WEEK";
                const status = inv.appointmentStatus === "MISSED" ? "MISSED" : "SCHEDULED";
