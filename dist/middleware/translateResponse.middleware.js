...
const ENUM_TRANSLATORS = {
    status: translations_1.translateStatus,
    caseType: translations_1.translateCaseType,
    reminderType: translations_1.translateReminderType,
    reminderStatus: translations_1.translateReminderStatus,
};
function collectStrings(data, jobs) {
...
function applyStaticTranslations(data, lang) {
    if (data === null || data === undefined)
        return;
    if (Array.isArray(data)) {
        for (const item of data)
            applyStaticTranslations(item, lang);
        return;
    }
    if (typeof data === "object") {
        for (const key of Object.keys(data)) {
            ...
            if (key === "gender" && typeof val === "string") {
                data.genderLabel = (0, translations_1.t)(`gender.${val}`, lang);
            }
        ...
        res.json = function (body) {
            ...
            if (!body || !body.success || !body.data) {
                ...
                if (body?.message) {
                    ...
                    if (msgKey)
                        body.message = (0, translations_1.t)(msgKey, lang);
                }
            ...
            Promise.all([
                translateTexts.length > 0 ? (0, translations_1.translateBatch)(translateTexts, lang) : [],
                transliterateTexts.length > 0 ? (0, translations_1.transliterateBatch)(transliterateTexts, lang) : [],
            ])
