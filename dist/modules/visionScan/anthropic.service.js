...
exports.isAnthropicConfigured = exports.extractWithAnthropic = exports.generateRescanTip = void 0;
...
async function cropToForm(buffer, meta, skipAspectGuard = false) {
    ...
    const mW = meta.width || info.width;
    const mH = meta.height || info.height;
    const sX = mW / info.width;
    const sY = mH / info.height;
    const sorted = Buffer.from(data).sort();
    const thr = Math.max(180, sorted[Math.floor(sorted.length * 0.80)] - 20);
    const colW = new Float32Array(info.width);
    const rowW = new Float32Array(info.height);
    for (let r = 0; r < info.height; r++)
        for (let c = 0; c < info.width; c++)
            if (data[r * info.width + c] >= thr) {
                colW[c] += 1 / info.height;
                rowW[r] += 1 / info.width;
            }
    ...
    for (let r = info.height - 1; r >= 0; r--)
        if (rowW[r] >= EDGE_DENSITY) {
            maxR = r;
            break;
        }
    minC = Math.max(0, minC - PADDING);
    maxC = Math.min(info.width - 1, maxC + PADDING);
    minR = Math.max(0, minR - PADDING);
    maxR = Math.min(info.height - 1, maxR + PADDING);
    const cropFrac = 1 - ((maxR - minR) * (maxC - minC)) / (info.height * info.width);
    if (cropFrac < 0.30) {
        console.log("[Preprocessor] Background too small — skipping crop");
        return buffer;
    }
    const left = Math.max(0, Math.floor(minC * sX));
    const top = Math.max(0, Math.floor(minR * sY));
    const right = Math.min(mW, Math.ceil(maxC * sX));
    const bottom = Math.min(mH, Math.ceil(maxR * sY));
    ...
    if (!skipAspectGuard) {
        ...
        if (oA > 1 && Math.max(oA, cA) / Math.min(oA, cA) > 1.5) {
            console.log("[Preprocessor] Crop would distort landscape — skipping");
            return buffer;
        }
    }
...
async function preprocessForVision(imageBuffer, maxDimension = STD_MAX_DIM, quality = STD_QUALITY) {
    const origMeta = await (0, sharp_1.default)(imageBuffer).metadata();
    let buf = await (0, sharp_1.default)(imageBuffer).rotate().toBuffer();
    let meta = await (0, sharp_1.default)(buf).metadata();
    let wasPortrait = false;
    if (meta.height && meta.width && meta.height > meta.width) {
        wasPortrait = true;
        const cw90 = await (0, sharp_1.default)(buf).rotate(90).toBuffer();
        const cw270 = await (0, sharp_1.default)(buf).rotate(270).toBuffer();
    ...
    meta = await (0, sharp_1.default)(buf).metadata();
    const processed = await (0, sharp_1.default)(buf)
...
exports.generateRescanTip = generateRescanTip;
async function callAnthropic(base64Image, userPrompt, apiKey, tracker, opts = {}) {
    ...
    if (data.usage) {
        ...
        tracker.calls++;
        if (isHaiku) {
            tracker.haikuInput += inp;
            tracker.haikuOutput += out;
        }
        else {
            tracker.sonnetInput += inp;
            tracker.sonnetOutput += out;
        }
        tracker.cacheWriteTokens += cw;
        tracker.cacheReadTokens += cr;
        if (cr > 0)
            console.log(`[Anthropic] Cache hit: ${cr} tokens (~$${((cr / 1000000) * 2.70).toFixed(4)} saved)`);
    }
...
function calcTokenUsage(t) {
    ...
    const estimatedCostUSD = parseFloat(((t.sonnetInput / 1000000) * 3.00 +
        (t.sonnetOutput / 1000000) * 15.00 +
        (t.haikuInput / 1000000) * 0.80 +
        (t.haikuOutput / 1000000) * 4.00 +
        (t.cacheWriteTokens / 1000000) * 3.75 +
        (t.cacheReadTokens / 1000000) * 0.30).toFixed(6));
...
function toMonthAbbr(mm) {
    const title = mm.charAt(0).toUpperCase() + mm.slice(1).toLowerCase();
    if (VALID_MONTHS.includes(title))
        return title; // already a valid abbreviation
...
// ─── Retry Prompts ────────────────────────────────────────────────────────
const buildDDMMRetry = (label) => `Focus ONLY on the "${label}" section of this CANTrac diary page.
Read BOTH rows:
...
const buildDDRetry = (label, cur, plus1) => plus1 ? `Focus on "${label}". Compare ONLY these two adjacent DD bubbles: day ${cur} and day ${plus1}. Which is DARKER?
...
const buildMMRetry = (label, hasData) => `Focus on "${label}". MM row — 12 bubbles: Jan Feb Mar Apr May Jun Jul Aug Sep Oct Nov Dec. Count N empty left → N=0→Jan…N=11→Dec.${hasData ? " Other fields in this section are filled, so a month IS selected." : ""} Return null only if section is completely blank.
Respond: { "mm": "<Jan|...|Dec|null>", "confidence": "high|medium|low" }`;
const buildYYBothRetry = () => `Look at the YEAR rows in BOTH appointment sections on this CANTrac schedule page.
...
const buildYYSingleRetry = (label) => `Look at the YEAR row in the "${label}" section. 3 bubbles: LEFTMOST=2026, MIDDLE=2027, RIGHTMOST=2028. Which ONE is SOLID dark inside?
Respond: { "yy": "<2026|2027|2028>", "confidence": "high|medium|low" }`;
const buildSecondSectionRetry = (fDD, fMM, fYY) => `Re-read ONLY the SECOND ATTEMPT (bottom) section. First appointment: ${fMM} ${fDD}, ${fYY}. Expect second attempt year to be ${fYY} (or at most ${parseInt(fYY) + 1}).
...
async function crossValidate(base64, apiKey, fields, tracker, warnings) {
    ...
    const issues = [];
    if (["Completed", "Missed", "Cancelled"].includes(fSt ?? "") && fYY && parseInt(fYY) > cur)
        issues.push("first_yy_future");
    if (fMM && sMM && fMM === sMM)
        issues.push("same_mm");
    if (fYY && sYY && fYY === sYY && ["Completed", "Missed", "Cancelled"].includes(fSt ?? ""))
    ...
    if (issues.includes("first_yy_future"))
        notes.push(`ERROR: Status '${fSt}' but year ${fYY} is in the future (current: ${cur}). LEFTMOST bubble (2026) is almost certainly correct.`);
    if (issues.includes("same_mm"))
        notes.push(`ERROR: Both sections show the same month (${fMM}). Each section has its OWN separate MM row.`);
    if (issues.includes("same_yy_terminal") && !issues.includes("first_yy_future"))
        notes.push(`ERROR: Both sections show year ${fYY} when first is '${fSt}'. Read each section's YY row independently.`);
...
Respond: { "reasoning": { "first_mm":"...", "first_yy":"...", "second_mm":"...", "second_yy":"..." }, "first_mm":"...", "first_yy":"...", "second_mm":"...", "second_yy":"..." }`;
    try {
        ...
        const applied = [];
        if (c.first_mm && VALID_MONTHS.includes(c.first_mm) && c.first_mm !== fMM) {
            fields["first_appointment_mm"] = { value: c.first_mm, confidence: "medium" };
            applied.push(`first_mm=${c.first_mm}`);
        }
        if (c.first_yy && VALID_YEARS.includes(String(c.first_yy))) {
            ...
            if (cy !== fYY && (parseInt(cy) <= cur && parseInt(fYY || "0") > cur || issues.includes("same_yy_terminal"))) {
                fields["first_appointment_yy"] = { value: cy, confidence: "medium" };
                applied.push(`first_yy=${cy}`);
            }
        }
        if (c.second_mm && VALID_MONTHS.includes(c.second_mm) && c.second_mm !== sMM) {
            fields["second_attempt_mm"] = { value: c.second_mm, confidence: "medium" };
            applied.push(`second_mm=${c.second_mm}`);
        }
        if (c.second_yy && VALID_YEARS.includes(String(c.second_yy)) && String(c.second_yy) !== sYY) {
            fields["second_attempt_yy"] = { value: String(c.second_yy), confidence: "medium" };
            applied.push(`second_yy=${c.second_yy}`);
        }
...
async function checkFormValidity(lowRes, apiKey, tracker) {
    try {
        const r = await callAnthropic(lowRes, `Is this a CANTrac breast cancer tracking diary page? A valid page has: pink/red header banner, bubble rows, page number at top center, ribbon symbol, QR code.
Respond: { "isValid": true|false, "reason": "<one sentence>" }`, apiKey, tracker, { model: HAIKU_MODEL, maxTokens: 256 });
        if (typeof r.isValid === "boolean")
            return { valid: r.isValid, reason: r.reason };
    }
    catch (err) {
...
async function assessImageQuality(lowRes, apiKey, tracker, warnings) {
    try {
        const q = await callAnthropic(lowRes, `Assess photographic quality of this CANTrac diary page photo.
Check: patterned/textured background, severe angle/skew, blur/out-of-focus, poor lighting/glare, form edges cut off.
Respond: { "patterned_background":bool, "severe_angle":bool, "blurry":bool, "poor_lighting":bool, "form_cut_off":bool, "overall_quality":"good"|"acceptable"|"poor" }`, apiKey, tracker, { model: HAIKU_MODEL, maxTokens: 256 });
        const issues = [];
        if (q.patterned_background)
            issues.push("diary page is on a patterned background — use a plain surface");
        if (q.severe_angle)
        ...
        if (issues.length > 0) {
            warnings.push("Image quality issues detected: " + issues.join("; ") + " — retake recommended for accuracy");
            console.log(`[Anthropic] Quality issues: ${issues.join(", ")}`);
        }
        if (q.overall_quality === "poor")
            warnings.push("Overall image quality is poor — retake the photo");
        else if (q.overall_quality === "acceptable" && issues.length > 0)
...
async function extractWithAnthropic(imageBuffer, pageNumber, pageTitle, questions) {
    ...
    const processedMeta = await (0, sharp_1.default)(mainPrep.buffer).metadata();
    const imageMetadata = {
        ...
        for (const { prefix, label } of SECTIONS) {
            ...
            if (ddBad && !nearEnd && mmBad) {
                ...
                try {
                    ...
                    if (mv)
                        rawResult.fields[mmKey] = { value: mv, confidence: c.mm_confidence || "medium" };
                    else
                        warnings.push(`MM for ${label} could not be read — manual verification required`);
                }
                catch (e) {
                    console.log(`[Anthropic] DD+MM retry failed: ${e.message}`);
                }
                continue;
            }
            if (ddBad) {
                ...
                try {
                    ...
                    if (r.dd) {
                        ...
                        if (nearEnd && padded === ddF?.value && p1Str && plus1 > ddVal) {
                            ...
                            rawResult.fields[ddKey] = { value: padded, confidence: r.confidence || "medium" };
                        }
                        if (nearEnd)
                            nearEndApplied.add(ddKey);
                    }
                }
                catch (e) {
                    console.log(`[Anthropic] DD retry failed: ${e.message}`);
                }
            }
            if (mmBad) {
        ...
        if (fYYBad && sYYBad) {
            ...
            try {
                ...
                if (c.second_yy && VALID_YEARS.includes(String(c.second_yy)))
                    rawResult.fields["second_attempt_yy"] = { value: String(c.second_yy), confidence: c.second_yy_confidence || "medium" };
            }
            catch (e) {
                console.log(`[Anthropic] YY combined retry failed: ${e.message}`);
            }
        }
        else {
            for (const { key, label, field } of [
                ...
                try {
                    ...
                    console.log(`[Anthropic] YY retry failed: ${e.message}`);
                }
            }
        }
        // Second section retry (year gap / terminal status)
        const fDD = rawResult.fields["first_appointment_dd"]?.value;
        const fMM = rawResult.fields["first_appointment_mm"]?.value;
        const fYY = rawResult.fields["first_appointment_yy"]?.value;
        const sYY = rawResult.fields["second_attempt_yy"]?.value;
        const sDd = rawResult.fields["second_attempt_dd"]?.value;
        const sMm = rawResult.fields["second_attempt_mm"]?.value;
            ...
            try {
                ...
                if (yearGap && retryGap > origGap && retryGap > 1)
                    ...
                    for (const k of ["second_attempt_dd", "second_attempt_mm", "second_attempt_yy", "second_attempt_status"]) {
                        if (k === "second_attempt_dd" && nearEndApplied.has(k))
                            continue;
                        if (retried[k]?.value !== null && retried[k]?.value !== undefined) {
                            if (retried[k].value !== rawResult.fields[k]?.value)
    ...
    const cantracFields = rawResult.fields || {};
    const extraction = {};
    if (isSchedule) {
        ...
        const datePfx = ["first_appointment", "second_attempt"];
        const stsPfx = ["first_appointment_status", "second_attempt_status"];
        for (let i = 0; i < dateQs.length; i++) {
            ...
            const minConf = Math.min(CONFIDENCE_MAP[cantracFields[`${pfx}_dd`]?.confidence ?? ""] ?? 0, CONFIDENCE_MAP[cantracFields[`${pfx}_mm`]?.confidence ?? ""] ?? 0, CONFIDENCE_MAP[cantracFields[`${pfx}_yy`]?.confidence ?? ""] ?? 0);
        ...
        if (yesNoQs[0] && nar) {
            let val = nar.value;
            if (val)
                val = val.toLowerCase().startsWith("y") ? "yes" : val.toLowerCase().startsWith("n") ? "no" : val;
        ...
        for (const q of questions) {
            ...
            if (raw) {
                ...
                if (q.type === "yes_no" && value) {
                    ...
                    if (["yes", "हाँ", "y", "true"].includes(l))
                        value = "yes";
                    else if (["no", "नहीं", "n", "false"].includes(l))
                        value = "no";
                }
                extraction[q.id] = { value, confidence: CONFIDENCE_MAP[raw.confidence ?? "medium"] ?? 0.75 };
                if (raw.confidence === "low")
                    warnings.push(`Low confidence on "${q.id}": ${q.text}`);
            }
            else {
    ...
    const allRescanReasons = [...warnings.filter(w => w.includes("quality") || w.includes("retake") || w.includes("patterned") ||
    ...
    if (lowConfCount > 0)
        allRescanReasons.unshift(`${lowConfCount} field(s) have low confidence — verify manually`);
