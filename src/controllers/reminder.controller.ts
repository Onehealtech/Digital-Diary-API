    ...
    try {
        ...
        if (isHindi) {
            const [hiTitle, hiAlertMessage, hiType, hiDoctorMessage] = await Promise.all([
                translateText("New Appointment Reminder", "hi"),
                translateText(enAlertMessage, "hi"),
                translateText(type, "hi"),
...
export const resendReminder = async (
    ...
    try {
        ...
        if (reminder.patient) {
            ...
            if (isHindi) {
                const [hiTitle, hiAlertMessage, hiType, hiDoctorMessage] = await Promise.all([
                    translateText("Appointment Reminder Updated", "hi"),
                    translateText(enAlertMessage, "hi"),
                    translateText(reminder.type, "hi"),
                    translateText(doctorMessage, "hi"),
                ]);
