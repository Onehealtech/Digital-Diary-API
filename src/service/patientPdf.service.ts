...
const LOGO_PATH = path.resolve(__dirname, "../assets/canTRAC-Logo.png");
...
// ── Type mirrors (kept local to avoid circular imports) ──────────────────────
...
interface PatientAllData {
    ...
    patient: {
        ...
        testCompletionPercentage?: number | null;
        totalTestsPrescribed?: number | null;
        registeredDate?: string;
        lastDiaryScan?: string;
        createdAt?: string;
        doctor?: DoctorInfo | null;
    };
    subscription?: {
    ...
    scanResults?: PatientScanResult[];
    doctorHistory?: DoctorHistoryEntry[];
}

// ── PDF builder ───────────────────────────────────────────────────────────────

export async function buildPatientPDF(data: PatientAllData): Promise<Buffer> {
    return new Promise((resolve, reject) => {
        ...
        const rgb = (c: { r: number; g: number; b: number }) =>
            doc.fillColor([c.r, c.g, c.b]).strokeColor([c.r, c.g, c.b]);

        const guard = (neededPt: number) => {
        ...
        const addFooter = () => {
            ...
            doc
                ...
                .font("Helvetica")
                .text(
                    "CanTRAC by OneHeal Technologies \u2014 Confidential Patient Health Record",
                    M + 18,
                    footerY - 4,
                    { lineBreak: false }
                )
        ...
        interface CellData { text: string; x: number; w: number }
        const tableRow = (cells: CellData[], isOdd: boolean) => {
            guard(7 * MM);
            if (isOdd) {
        ...
        if (p.treatmentPlan)  row("Treatment:",  p.treatmentPlan);
        ...
        if (sub) {
            ...
            row("Pages Used:", `${sub.pagesUsed ?? 0} / ${pagesLabel}`,
                                                           { label: "Paid:",          value: sub.paidAmount ? `\u20B9${sub.paidAmount}` : "\u2014" });
            row("Start Date:", fmtDate(sub.startDate),    { label: "End Date:",      value: fmtDate(sub.endDate) });
            row("Scan:",       sub.scanEnabled ? "Enabled" : "Disabled",
                                                           { label: "Manual Entry:",  value: sub.manualEntryEnabled ? "Enabled" : "Disabled" });
        } else {
        ...
        const tests = (p.prescribedTests as PrescribedTest[]) || [];
        if (tests.length > 0) {
        ...
        doc.fontSize(7.5).font("Helvetica").fillColor([GRAY.r, GRAY.g, GRAY.b])
           .text(
               "This document contains confidential patient health information. Keep it secure.",
               M,
               curY + 9 * MM,
...
export async function generateAndUploadPatientPDF(
    data: PatientAllData,
    patientId: string,
    diaryId?: string
): Promise<string> {
    ...
    const url = await uploadBufferToS3(pdfBuffer, "application/pdf", s3Key);
    return url;
}
