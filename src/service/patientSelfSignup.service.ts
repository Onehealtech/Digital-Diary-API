...
export async function sendSignupOtp(
  phone: string
): Promise<{ message: string; isExistingUser: boolean; sessionId: string }> {
...
export async function verifySignupOtp(
  ...
  if (!session) {
    throw new AppError(401, "Invalid or expired session. Please request a new OTP.");
  }

  if (new Date() > session.expiresAt) {
    otpSessionStore.delete(sessionId);
    throw new AppError(401, "Session expired. Please request a new OTP.");
  }
...
export async function listAvailableDoctors(params: {
  ...
  const patientCounts = await Patient.count({
    where: { doctorId: { [Op.in]: doctorIds } },
    group: ["doctorId"],
  });

  const countMap = new Map(
    (patientCounts as unknown as { doctorId: string; count: string }[]).map(
      (r) => [r.doctorId, Number(r.count)]
    )
  );
