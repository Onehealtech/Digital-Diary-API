...
exports.advancedAnalysisRepository = exports.AdvancedAnalysisRepository = void 0;
 ...
 */
function getAnswer(obj, key) {
    const val = obj[key];
    if (val === null || val === undefined)
        return null;
    if (typeof val === "object" && !Array.isArray(val) && "answer" in val) {
        return val.answer;
    }
    return val;
}
...
// -----------------------------------------------------------------------
function parseAppointmentStatus(val) {
 ...
 * Returns { status, date }.
 */
function resolveScheduleStatus(sr) {
...
// -----------------------------------------------------------------------
function buildInvestigationData(key, scansByPage) {
    ...
    if (config.donePage !== undefined) {
        ...
        for (const sr of donePageScans) {
            if (config.doneQIds.some((qId) => getBool(sr, qId)))
                testDone = true;
            if (getBool(sr, config.reportQId))
                reportCollected = true;
            if (getBool(sr, config.problemQId))
                problemFlagged = true;
            ...
            if (!isNaN(problemQNum) && getBool(sr, `q${problemQNum + 1}`))
                problemFlagged = true;
        }
    }
...
// Stage deriver
// -----------------------------------------------------------------------
function deriveStage(hasInvestigationData, hasTreatmentPlan, hasNact, hasSurgery) {
    if (hasSurgery)
        return "SURGERY";
    if (hasNact)
        return "NACT";
...
// Issues deriver
// -----------------------------------------------------------------------
function deriveIssues(investigations, treatmentPlan, nact) {
    const issues = [];
    // MISSED_APPT: any investigation with MISSED appointment status
    const hasMissedAppt = Object.values(investigations).some((inv) => inv.appointmentStatus === "MISSED");
    if (hasMissedAppt)
        issues.push("MISSED_APPT");
    // NO_REPORT: ordered + done but report not collected
    const hasNoReport = Object.values(investigations).some((inv) => inv.ordered && inv.testDone && !inv.reportCollected);
    if (hasNoReport)
        issues.push("NO_REPORT");
    // CHEMO_ISSUE: tumor growing or unable to complete
    if (nact.tumorGrowing || nact.unableToComplete)
        issues.push("CHEMO_ISSUE");
    // CLIPS_MISSING: chemo started but clips missing
    if (nact.chemoStartedClipsMissing)
        issues.push("CLIPS_MISSING");
    // NO_PLAN: treatment not decided
    if (treatmentPlan.notDecidedYet)
        issues.push("NO_PLAN");
    return issues;
}
// -----------------------------------------------------------------------
...
// -----------------------------------------------------------------------
class AdvancedAnalysisRepository {
    /**
     ...
     */
    mapToPatientAnalysisRow(patient, scans) {
        const p = patient;
        ...
        const hasTreatmentPlan = treatmentPlan.nact ||
        ...
        const currentStage = deriveStage(hasInvestigationData, hasTreatmentPlan, hasNact, hasSurgery);
        // ---- Derive issues ----
        const issues = deriveIssues(investigations, treatmentPlan, nact);
        // ---- Last activity date ----
        const latestScan = typedScans.reduce((latest, s) => {
            if (!latest)
                return s;
        ...
        const lastActivityDate = latestScan?.scannedAt
    ...
    applyFilters(rows, filter) {
        return rows.filter((row) => {
            ...
            if (filter.ageMin !== undefined && row.age < filter.ageMin)
                return false;
            if (filter.ageMax !== undefined && row.age > filter.ageMax)
                return false;
            // Sex
            if (filter.sex !== "ALL") {
                const rowSex = row.sex.toUpperCase();
                if (rowSex !== filter.sex)
                    return false;
            }
            ...
            if (filter.investigations) {
                for (const [key, status] of Object.entries(filter.investigations)) {
                    ...
                    if (status === "ORDERED_NOT_SCHEDULED") {
                        if (!(inv.ordered && inv.appointmentStatus === "NONE"))
                            return false;
                    }
                    else if (status === "SCHEDULED") {
                        if (inv.appointmentStatus !== "SCHEDULED")
                            return false;
                    }
                        ...
                        if (inv.appointmentStatus !== "MISSED")
                            return false;
                    }
                    else if (status === "CANCELLED") {
                        if (inv.appointmentStatus !== "CANCELLED")
                            return false;
                    }
                    else if (status === "PROBLEM_FLAGGED") {
                        if (!inv.problemFlagged)
                            return false;
                    }
                }
            }
            ...
            if (filter.nactPlanned !== "ANY") {
                ...
                if (row.treatmentPlan.nact !== expected)
                    return false;
            }
            // Surgery type
            if (filter.surgeryType !== "ANY") {
                if (filter.surgeryType === "NOT_PLANNED") {
                    if (row.treatmentPlan.surgeryType !== null)
                        return false;
                }
                else {
                    if (row.treatmentPlan.surgeryType !== filter.surgeryType)
                        return false;
                }
            }
            ...
            if (filter.radiationBooked !== "ANY") {
                ...
                if (filter.radiationBooked === "YES" && rs !== "SCHEDULED" && rs !== "COMPLETED")
                    return false;
                if (filter.radiationBooked === "NO" && (rs === "SCHEDULED" || rs === "COMPLETED"))
                    return false;
                if (filter.radiationBooked === "MISSED" && rs !== "MISSED")
                    return false;
                if (filter.radiationBooked === "CANCELLED" && rs !== "CANCELLED")
                    return false;
            }
            ...
            if (filter.surgeryAdmission !== "ANY") {
                ...
                if (filter.surgeryAdmission === "YES" && as_ !== "SCHEDULED" && as_ !== "COMPLETED")
                    return false;
                if (filter.surgeryAdmission === "NO" && (as_ === "SCHEDULED" || as_ === "COMPLETED"))
                    return false;
                if (filter.surgeryAdmission === "MISSED" && as_ !== "MISSED")
                    return false;
                if (filter.surgeryAdmission === "CANCELLED" && as_ !== "CANCELLED")
                    return false;
            }
            return true;
        });
    ...
    applySorting(rows, sortBy) {
        ...
        switch (sortBy) {
            ...
            case "latest_activity":
                sorted.sort((a, b) => new Date(b.lastActivityDate).getTime() -
...
exports.AdvancedAnalysisRepository = AdvancedAnalysisRepository;
exports.advancedAnalysisRepository = new AdvancedAnalysisRepository();
