...
exports.getDoctorEarnings = exports.getVendorEarnings = exports.processPaymentSuccess = exports.createDiaryOrder = void 0;
...
//     };
// };
const createDiaryOrder = async (params) => {
    const { patientId, doctorId, vendorId, amount, generatedDiaryId, customerPhone, customerName, customerEmail, orderNote, } = params;
    // 1️⃣ Validate entities
    const patient = await Patient_1.Patient.findByPk(patientId);
    if (!patient)
        throw new Error("Patient not found");
    const vendor = await Appuser_1.AppUser.findByPk(vendorId);
    if (!vendor)
        throw new Error("Vendor not found");
    ...
    const result = await Dbconnetion_1.sequelize.transaction(async (t) => {
        ...
        await Diary_1.Diary.create({ id: generatedDiaryId,
            patientId,
            doctorId,
            vendorId,
            status: "pending",
        ...
        const generateDiaryId = await GeneratedDiary_1.GeneratedDiary.findOne({
...
exports.createDiaryOrder = createDiaryOrder;
/**
 ...
 */
const processPaymentSuccess = async (orderId, paymentMethod, webhookPayload) => {
    return await Dbconnetion_1.sequelize.transaction(async (t) => {
        ...
        const splitTxn = await SplitTransaction_1.SplitTransaction.findOne({
        ...
        if (splitTxn) {
            ...
            const platformAdmin = await Appuser_1.AppUser.findOne({
                where: { role: "SUPER_ADMIN" },
                transaction: t,
            });
 ...
 */
const getVendorEarnings = async (vendorId, startDate, endDate) => {
    const whereClause = {
        vendorId,
        status: "PAID",
    };
    ...
    const totals = await SplitTransaction_1.SplitTransaction.findOne({
        attributes: [
            [sequelize_2.Sequelize.fn("SUM", sequelize_2.Sequelize.col("vendorAmount")), "totalEarnings"],
            [sequelize_2.Sequelize.fn("COUNT", sequelize_2.Sequelize.col("SplitTransaction.id")), "totalOrders"],
        ],
        include: [
            {
                model: Order_1.Order,
 ...
 */
const getDoctorEarnings = async (doctorId, startDate, endDate) => {
    const whereClause = {
        doctorId,
        status: "PAID",
    };
    ...
    const totals = await SplitTransaction_1.SplitTransaction.findOne({
        attributes: [
            [sequelize_2.Sequelize.fn("SUM", sequelize_2.Sequelize.col("doctorAmount")), "totalEarnings"],
            [sequelize_2.Sequelize.fn("COUNT", sequelize_2.Sequelize.col("SplitTransaction.id")), "totalOrders"],
        ],
        include: [
            {
