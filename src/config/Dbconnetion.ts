...
export const initializeDatabase = async (): Promise<void> => {
  try {
    ...
    // Add scanCount column to bubble_scan_results
    await sequelize.query(`
      DO $$
      BEGIN
        IF NOT EXISTS (
          SELECT 1 FROM information_schema.columns
          WHERE table_name = 'bubble_scan_results' AND column_name = 'scanCount'
        ) THEN
          ALTER TABLE "bubble_scan_results" ADD COLUMN "scanCount" INTEGER NOT NULL DEFAULT 1;
        END IF;
      END
      $$;
    `).catch((err: unknown) => {
      console.warn('⚠️ bubble_scan_results.scanCount migration warning:', err instanceof Error ? err.message : err);
    });
